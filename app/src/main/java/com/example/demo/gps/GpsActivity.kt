        ...
        if (EasyPermissions.hasPermissions(this, *perms)) {
            ...
            lifecycleScope.launch() {
                val nearByResult = withContext(Dispatchers.IO) {
                    RetrofitClient.instance.query8864Api.nearBusByLocation("find_zhans_by_jwd", "beijing", "116.282071", "40.048732", "10")
                }
                val realTimeInfo = withContext(Dispatchers.IO) {
                    // fetchBjBusApiData()
                    // https://api.8684.cn/bus_position_1.php?ecity=beijing&line_name=963路&to_station_name=龙泉驾校&appkey=Yv9cL8wTwZgr
                    RetrofitClient.instance.query8864Api.realTimeBusInfo("beijing", "963路", "龙泉驾校")
                }
                Log.d("yyyyyy", "nearBusByLocation data $nearByResult")
                Log.d("yyyyyy", "realTimeInfo $realTimeInfo")
            }
        } else {
